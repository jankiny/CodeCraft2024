...
struct Boat {
    ...
    void FindSuitableBerth() {
            ...
            if (g_berths[it->second.second].storedGoods.size() < 0) {
                ...
                if (this->status == 2) {
                    ...
                    if (g_berths[it->second.second].leftValueBerth >= 500) {
                        ...
                        double choiceValue =
                                g_berths[it->second.second].leftValueBerth * g_berths[it->second.second].loadingSpeed;
        ...
        if (maxBerthId >= 0) {
            logInfo << "Status " << this->status << " Change the Target Berth From " << sourceBerthId << " To "
                    << maxBerthId << endl;
...
double CalcWaitValueForStateTwo(int berthId, int capacity) {
    ...
    if (g_berths[berthId].boatQueue.empty()) {
        ...
        double waitMaxTime = (1.0 * g_boats[curBoatInBerth].capacity / g_berths[berthId].loadingSpeed) +
                             g_berths[berthId].transportTime + (capacity * 1.0 / g_berths[berthId].loadingSpeed);
        // logInfo << 3 << endl;
        // 即将装到本艘船上的货物时间价值均值
...
double CalcMoveValueForStateTwo(int targetIndex, int sourceIndex, int capacity) {
    ...
    double moveMaxTime =
            500 + g_berths[targetIndex].transportTime + (capacity * 1.0 / g_berths[sourceIndex].loadingSpeed);
    ...
    if (moveBoatInBerth) {
        ...
        moveMaxTime = moveMaxTime + (g_boats[g_berths[targetIndex].boatQueue.front()].capacity * 1.0 /
                                     g_berths[targetIndex].loadingSpeed);
...
double CalcMoveValueForStateZero(int targetIndex, int sourceIndex, int capacity) {
    ...
    if (moveBoatInBerth) {
        ...
        moveMaxTime = (moveMaxTime * g_berths[targetIndex].boatQueue.size()) +
                      (g_boats[g_berths[targetIndex].boatQueue.front()].capacity * 1.0 /
                       g_berths[targetIndex].loadingSpeed);
                ...
                if (g_boats[i].capacity > 0) {
                    ...
                    if (g_berths[g_boats[i].berthId].storedGoods.size() > 0) {
                        ...
                        for (int k = 0; k < g_berths[g_boats[i].berthId].loadingSpeed; k++) {
                            ...
                            // 去除泊位持有的该货物
                            g_berths[g_boats[i].berthId].storedGoods.erase(
                                    g_berths[g_boats[i].berthId].storedGoods.begin());
                            // 泊位价值总量排序动态更新
                            // PrintBerthValueRank();
                            for (auto it = berth_value_rank.begin(); it != berth_value_rank.end(); ++it) {
                                if (it->second.second == g_boats[i].berthId) {
                                    lock_index berthStatus(g_berths[g_boats[i].berthId].totalValueBerth,
                                                           g_boats[i].berthId);
                                    berth_value_rank.insert(
                                            make_pair(g_berths[g_boats[i].berthId].leftValueBerth, berthStatus));
                                    berth_value_rank.erase(it);
                                    break;
                                }
                            }
