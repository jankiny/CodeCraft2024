...
const int     LIMIT_LOAD_FRAME            =   TOTAL_FRAME - 650;                  // 终止装货的帧数
...
struct Boat {
    ...
    void FindSuitableBerth() {
        ...
        // this->berthId = -1;
        // TODO 考虑泊位到虚拟点的TransportTime, 采用均值衡量, 价值x时间来权衡
        // 记录最大货物价值泊位的ID
        int maxIndex = -1;
        // 若map按value降序,则只取第一个即可, 就不考虑循环遍历了
        // 如果还要考虑TransportTime则,需要改进
        int maxValue = -1000;
        // PrintAvailableBerth();
        // for (auto it = avaliable_berth.begin(); it != avaliable_berth.end(); ++it) {
            // 若可用Berth中goods value 为零
        // logInfo << "############################" << endl;
        // PrintAvailableBerth();
        // logInfo << ""
        auto it = --berth_value_rank.end();
        if (g_berths[it->second.second].storedGoods.size() < 0) {
            // TODO 权衡TransportTime与Berth位移Time, 来考虑是否为船指定Berth
            logInfo << "Error Avaliable Berth " << it->second.second << " Goods Value Negative " << endl;
        } else {
            if (g_berths[it->second.second].storedGoods.size() > 0) {
                // 找到货物价值最大的可用泊位
                // if (g_berths[it->second].totalValueBerth > maxValue) {
                maxValue = g_berths[it->second.second].totalValueBerth;
                maxIndex = it->second.second;
                // }
                // logInfo << "maxValue " << maxValue << " in Berth " << maxIndex << endl;
            }
        }
        // 若得到了最大货物价值的可用泊位
        if (maxIndex != -1) {
            // 权衡手段: 如果当前船处于等待状态, 则考虑移动到其他泊位的时间加上TransportTime加上预留上货时间
            if (this->status == 2) {

            } else { // 船处于虚拟点, 直接分配目标泊位
                this->berthId = g_berths[maxIndex].id;
            }
            // 让多艘船同时朝一个位置行驶,这样一艘船走了马上就可以让另一艘开始运
            // avaliable_berth.erase(it); // 维护available_berth
            logInfo << "Found berth "<< g_berths[maxIndex].id << " for boat " << this->id << endl;
        } else {
            this->berthId = -1;
            logInfo << "NOT Found berth for boat " << this->id << endl;
        }
        // PrintAvailableBerth();
        // logInfo << "############################" << endl;
    }
            ...
            if (this->targetBerth != nullptr && InBerth(this->targetBerth->p ,nextPoint) && this->goods == 1) {
                ...
                for (auto it = berth_value_rank.begin(); it != berth_value_rank.end(); ++ it) {
                    if (it->second.second == this->targetBerth->id) {
                        ...
                        lock_index berthStatus(-1, this->targetBerth->id);
                        // logInfo << "locked " << berthStatus.first << " berth " << berthStatus.second << endl;
                        berth_value_rank.insert(make_pair(this->targetBerth->totalValueBerth, berthStatus));
                        // PrintBerthValueRank();
...
void HandleFrame(int frame) {
    ...
    for (int i = 0; i < BOAT_NUM; i++) {    // 船(Boat)
        ...
        // // 打印船状态
        // logInfo << "Boat\t\tCapacity\t\tTotalValue\t\tBerthID\t\tStatus\t\tFinishFrame" << endl;
        // logInfo << g_boats[i].id << "\t\t" << g_boats[i].capacity << "\t\t" << g_boats[i].totalValueBoat << "\t\t" 
        //             << g_boats[i].berthId << "\t\t"<< g_boats[i].status << "\t\t" << g_boats[i].finishTransportFrame << endl;
                ...
                if (g_boats[i].capacity > 0) {
                    ...
                    // 针对当前船只的装载速度, 装货
                        for (int k = 0; k < g_berths[g_boats[i].berthId].loadingSpeed; k ++) {
                            ...
                            for (auto it = berth_value_rank.begin(); it != berth_value_rank.end(); ++ it) {
                                if (it->second.second == g_boats[i].berthId) {
                                    lock_index berthStatus(-1, g_boats[i].berthId);
                                    berth_value_rank.insert(make_pair(g_berths[g_boats[i].berthId].totalValueBerth, berthStatus));
                                    berth_value_rank.erase(it);
                                    break;
                                }
                            }
                            ...
                            if (g_berths[g_boats[i].berthId].storedGoods.size() == 0) {
                                //TODO find suitable berth for current boat, consider about the transport time and value
                                break;
                            }
                        }
                    } else { // 泊位没货了
                        ...
                        if (g_boats[i].berthId != -1) {
                            ...
                            if (frame + 500 + g_berths[g_boats[i].berthId].transportTime < TOTAL_FRAME) {
                                printf("ship %d %d\n", i, g_boats[i].berthId);
                            } else { // 否则直接将船移动到虚拟点
                                printf("go %d\n", i);
                            }
...
void PrintBerthValueRank() {
    logInfo << "Value\t\tLocked\t\tIndex -----> Berth Value Rank" << endl;
    for (auto it = berth_value_rank.begin(); it != berth_value_rank.end(); ++it) {
